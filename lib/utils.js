...
function createMarkdownFile(name, folder, content) {
...
function itemsToMarkdown(folder, items, mapping, downloadThumbnail) {
    let count = 1
    cleanContentDir(folder, /^item/)

    items.forEach(item => {
        const _folder = path.join(folder, `item_${count}`)
        ...
        if (downloadThumbnail) {
            ...
            const writer = fs.createWriteStream(path.join(_folder, "thumbnail"))
