...
- **Legacy blog redirects** — The blog previously lived on Tumblr and Jekyll/Firebase where posts were served from the site root (e.g. `/my-article` instead of `/blog/my-article`). Old links shared on social media still use those root paths. A build-time Astro integration generates a Netlify `_redirects` file that `301`-redirects every post slug from the root to its canonical `/blog/` location. The integration scans the generated output, so new posts are covered automatically.
