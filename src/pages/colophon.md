...
- **RSS feed** — A full blog feed at [/rss.xml](/rss.xml) ensures blog content is syndication-friendly and subscribable from any RSS reader.
- **Atom feed** — A microblog feed at [/microblog.xml](/microblog.xml) exposes the latest posts from the author's self-hosted [ATproto](https://atproto.com) PDS ([`social.l3x.in`](https://social.l3x.in)) as a standard [Atom 1.0](https://www.rfc-editor.org/rfc/rfc4287) document. The XML is generated from scratch (no dependency on `@astrojs/rss`, which only emits RSS 2.0) using the same `escapeHtml` sanitiser that protects all other user-controlled content on the site. The feed is discovered automatically via a `<link rel="alternate" type="application/atom+xml">` tag in every page's `<head>` and is listed on the [/follow](/follow) page.
